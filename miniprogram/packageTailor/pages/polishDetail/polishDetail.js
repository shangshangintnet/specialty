...
Page({
  ...
  changeLogisticsNum(e) {
    let key = "stone.logisticsNum"
    this.setData({
      [key]: e.detail.value
    })
  },
  changeStoreLogisticsNum(e) {
    let key = "stone.storeLogisticsNum"
    this.setData({
      [key]: e.detail.value
    })
  },
  ...
  confirm() {
    ...
    httphelper.api("carve/updatePolish", that.data.stone, function (serverdata) {
      if (serverdata.code == 200) {
  ...
  viewUserLogistics(e) {
    ...
    wx.navigateTo({
      url: '/pages/order/order_detail/express_detail/express_detail?type=4&id=' + this.data.stone.id,
    })
  },
  viewStoreLogistics(e) {
    ...
    wx.navigateTo({
      url: '/pages/order/order_detail/express_detail/express_detail?type=5&id=' + this.data.stone.id,
    })
  },
  /* 接收需求 */
  receive(e) {
  ...
  gokefu(e) {
    if (this.data.beUser) {
      ...
      if (that.data.stone.imageUrl != null && that.data.stone.imageUrl.length > 0)
        msg.image = that.data.stone.imageUrl[0];
      msg.path = "/packageTailor/pages/polishDetail/polishDetail?id=" + this.data.stone.id;
      let post = {};
