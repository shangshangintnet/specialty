...
Page({
  ...
  confirm() {
    ...
    httphelper.api("carve/updateInlay", that.data.stone, function (serverdata) {
      if (serverdata.code == 200) {
  ...
  viewUserLogistics(e) {
    ...
    wx.navigateTo({
      url: '/pages/order/order_detail/express_detail/express_detail?type=2&id=' + this.data.stone.id,
    })
  },
  viewStoreLogistics(e) {
    ...
    wx.navigateTo({
      url: '/pages/order/order_detail/express_detail/express_detail?type=3&id=' + this.data.stone.id,
    })
  },
  /* 接收需求 */
  receive(e) {
  ...
  gokefu(e) {
    if (this.data.beUser) {
      ...
      msg.path = "/packageTailor/pages/inlayDetail/inlayDetail?id=" + this.data.stone.id;
      let post = {};
