...
Page({
  ...
  getdata() {
    ...
    httphelper.api("carve/getCarve ", data, function (serverdata) {
  ...
  viewInlay(e) {
    app.curStone = this.data.inlaydata[e.currentTarget.dataset.index];
    wx.navigateTo({
      url: '../carveDetail/carveDetail',
    })
  },
  revoke(e) {
    let stone = that.data.inlaydata[e.currentTarget.dataset.index];
    wx.showModal({
      title: '撤销受理',
